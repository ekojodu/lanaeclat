		...
		<main className='services-page' ref={revealRef as any}>
			...
			<div className='process-banner'>
				{[
					...
					<span
						key={i}
						className={item === '→' ? 'process-arrow' : 'process-step'}
					>
						{item}
					</span>
				))}
					...
					<div className='services-list'>
						{treatments.map((t, i) => (
							...
							>
								...
								>
									<div className='service-left'>
										...
										<div>
											...
											{t.highlight && (
												<span className='service-tag'>{t.highlight}</span>
											)}
										</div>
									</div>
									<div className='service-right'>
										...
										<div
											className={`service-toggle ${active === t.id ? 'open' : ''}`}
										>
											<span />
											<span />
										</div>
								...
								<div
									className={`service-body ${active === t.id ? 'open' : ''}`}
								>
									<p>{t.description}</p>
						...
						{[
							{
								...
								desc: 'Every product we use is carefully selected for safety, efficacy, and skin compatibility.',
							},
							{
								icon: '🧪',
								title: 'Sterilized Tools',
								desc: 'All tools are thoroughly cleaned and sterilized before each session. Always.',
							},
							{
								icon: '🎓',
								title: 'Trained Hands',
								desc: 'Our treatments are performed by trained, passionate estheticians who care.',
							},
							{
								icon: '💌',
								title: 'Aftercare Support',
								desc: "We guide you through your at-home routine to prolong every session's results.",
							},
						].map((p, i) => (
							<div
								key={p.title}
								className={`promise-card reveal reveal-delay-${i + 1}`}
							>
			...
			<section
				...
				style={{ textAlign: 'center', background: 'var(--rose-pearl)' }}
			>
				<div className='container reveal'>
					<p className='section-label'>Ready?</p>
					<h2 style={{ marginBottom: '12px' }}>
						First-time clients get a{' '}
						<em style={{ color: 'var(--rose-deep)' }}>special offer</em>
					</h2>
					<p
						style={{
							color: 'var(--warm-grey)',
					...
					>
						Limited slots available. Book early to secure your spot and your
						glow.
					</p>
